...
Client.prototype.event = function(event_data) {
...
Client.prototype.sendEvent = function(event_data) {
  var send_data = "_e";

  if (event_data) {
    if (event_data.title && event_data.text) {
      send_data += '{' + event_data.title.length + ',' + event_data.text.length + '}';
      send_data += ':' + event_data.title + '|' + event_data.text;
    }

    if (event_data.date_happened) {
      send_data += '|d:' + event_data.date_happened;
    }

    if (event_data.hostname) {
      send_data += '|h:' + event_data.hostname;
    }
    if (event_data.priority) {
      send_data += '|p:' + event_data.priority;
    }
    if (event_data.alert_type) {
      send_data += '|t:' + event_data.alert_type;
    }

    if (event_data.tags && event_data.tags.length > 0) {
      send_data += '|#' + event_data.tags.join(',');
    }
